    ...
    >
      <ul className="md:flex md:gap-4 links__container">
        {links.map((link, index) => (
          <li key={index}  className="font-bold md:h-12 md:border-0 rounded transition ease-in-out duration-500 border-t border-b hover:bg-yellow-400 hover:text-primary border-white uppercase text-white h-16 flex items-center cursor-pointer">
            <Link className=" px-4" to={link.url}>
              {link.name}
            </Link>
          </li>
        ))}
